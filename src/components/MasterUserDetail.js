...
const MasterUserClients = ({ masterCode }) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/masteruser/${masterCode}/clients`, requestOptions)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      >
        <thead className="bg-gray-800">
          <tr>
            ...
            <th className="px-4 py-2">Master Code</th>
        ...
        <tbody>
          {clients.map((client, index) => (
            ...
            >
              ...
              <td className="px-4 py-2">{client.masterCode}</td>
