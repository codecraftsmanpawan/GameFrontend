...
const TenColorGame = () => {
    ...
    const fetchGameDetails = () => {
      ...
      fetch(`${baseURL}/admin/ongoing-game-details`, requestOptions)
        ...
        .then(result => {
          ...
          const filteredDetails = result.filter(game => game.mode === "tenColors");
          setGameDetails(filteredDetails);
          const currentGame = filteredDetails[0]; // Assuming only one game is relevant
          if (currentGame) {
            setCountdownDisplay(formatCountdown(currentGame.countdown));
            setIsToggleActive(currentGame.countdown <= 30); // Activate toggle button in the last 30 seconds
          }
        })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentGame) {
        setCountdownDisplay(formatCountdown(currentGame.countdown));
        setIsToggleActive(currentGame.countdown <= 30);
      }
    }, 1000);
  ...
  return (
    <div className="bg-gray-800 text-white p-6 rounded-lg max-w-2xl mx-auto my-6">
      <h2 className="text-xl"><strong>Current Winner: {currentGame.winnerColor}</strong></h2>
      ...
      <table className="min-w-full bg-gray-700 mb-4 mt-8">
        ...
        <tbody className="text-center">
          {colors.map((color, idx) => {
            const detail = currentGame.details.find(d => d.color === `Color${idx}`) || {};
